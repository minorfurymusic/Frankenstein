...
| OpenNutriTracker | **DENTRO DO MVP** — cobre item 2 da Definição de Pronto (código de barras) e é Flutter, mesma stack hipotética do shell | **PORT** (reimplementação informada, não link de código) — decidido em `docs/adr/005-licenciamento-distribuicao.md` (revisão 2): mantém o cliente Apache-2.0, fora do copyleft GPL-3.0 do OpenNutriTracker | GPL-3.0 | Nenhum de licença (PORT evita o copyleft); custo de engenharia de reimplementar em vez de linkar, não estimado | `docs/recon/opennutritracker.md` |
